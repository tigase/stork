...
public class SyncAdapter extends AbstractThreadedSyncAdapter {
	...
	public static long ensureGroupExists(Context context, Account account, String group) {
		...
		final Cursor cursor = resolver.query(Groups.CONTENT_URI, new String[] { BaseColumns._ID }, Groups.ACCOUNT_NAME
				+ "=? AND " + Groups.ACCOUNT_TYPE + "=? AND " + Groups.TITLE + "=?", new String[] { account.name, account.type,
				group }, null);
		if (cursor != null) {
			try {
	...
	private void deleteContacts(ContentResolver resolver, Account account, BatchOperation batchOperation) {
		// final Cursor c = resolver.query(RawContacts.CONTENT_URI, new String[]
		// {RawContacts._ID, RawContacts.SOURCE_ID},
		// RawContacts.ACCOUNT_TYPE+"='"+Constants.ACCOUNT_TYPE+"'", null,
		// null);
	...
	public void onPerformSync(Account account, Bundle extras, String authority, ContentProviderClient provider,
		...
		try {
			...
			while (c.moveToNext()) {
				...
				if (id == 0) {
					...
					// ContactOperations.updateExistingContact(context, id,
					// true, batchOperation);
		...
		// final ContactOperations contactOps =
		// ContactOperations.createNewContact(context, ri.getId(), account.name,
		// true, batchOperation);
