...
public class FetchMessageArchive
		implements Runnable {
	...
	public FetchMessageArchive(XMPPService xmppService, SessionObject sessionObject) {
	...
	private void fetchHistory(final Jaxmpp jaxmpp, final Chat chat, Date date) {
		...
		try {
			Criteria cr = new Criteria().setStart(date).setWith(chat.getWithJid());
							...
							try {
								...
								Uri uri = Uri.parse(
										ChatProvider.CHAT_HISTORY_URI + "/" + sessionObject.getUserBareJid() + "/" +
												chat.getWithJid().getBareJid());

								uri = xmppService.getContentResolver().insert(uri, values);
								xmppService.getApplicationContext()
										.getContentResolver()
										.notifyChange(
												ContentUris.withAppendedId(ChatProvider.OPEN_CHATS_URI, co.getId()),
												null);
							} catch (Exception e) {
