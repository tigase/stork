 ...
 * <p/>
 * Activities containing this fragment MUST implement the {@link OnRosterItemIteractionListener}
 * interface.
 */
public class RosterItemFragment extends Fragment {
    ...
    private Cursor loadData() {
        ...
        switch (s) {
            ...
            case "jid":
                ...
                break;
            case "presence":
                sort = DatabaseContract.RosterItemsCache.FIELD_STATUS + " DESC," + DatabaseContract.RosterItemsCache.FIELD_NAME + " COLLATE NOCASE ASC";
                break;
            default:
                sort = "";
        }
    ...
    public void onPrepareOptionsMenu(Menu menu) {
        ...
        ((MenuItem) menu.findItem(R.id.menu_roster_show_offline)).setChecked(v);

        String sort = sharedPref.getString("roster_sort", "presence");
        switch (sort) {
            case "name":
                ((MenuItem) menu.findItem(R.id.menu_roster_sort_name)).setChecked(v);
                break;
            case "presence":
                ((MenuItem) menu.findItem(R.id.menu_roster_sort_presence)).setChecked(v);
                break;
        }
    }
