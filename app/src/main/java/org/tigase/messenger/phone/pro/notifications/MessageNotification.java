...
public class MessageNotification {
	...
	private void _show(Context context, String senderName, String account, String chatJid, String messageBody,
					   PendingIntent resultPendingIntent, Bitmap avatar, Uri soundUri, long[] vibrationPattern,
					   int notificationId) throws JaxmppException {
		...
		NotificationCompat.Builder notifBuilder = new NotificationCompat.Builder(context).setContentTitle(
				context.getString(R.string.notification_new_message_from, senderName))
		...
		final String[] unreadChatsCols = new String[]{DatabaseContract.OpenChats.FIELD_ID, ChatProvider.FIELD_NAME,
													  ChatProvider.FIELD_UNREAD_COUNT, ChatProvider.FIELD_LAST_MESSAGE,
													  ChatProvider.FIELD_LAST_MESSAGE_TIMESTAMP,
													  ChatProvider.FIELD_LAST_MESSAGE_STATE};

		Cursor unreadChatsCursor = context.getContentResolver()
				.query(ChatProvider.OPEN_CHATS_URI, unreadChatsCols, ChatProvider.FIELD_UNREAD_COUNT + ">0", null,
		...
		NotificationCompat.Builder summaryNotificationBuilder = new NotificationCompat.Builder(context).setSmallIcon(
				R.drawable.ic_messenger_icon).setGroup(GROUP_KEY_MESSAGES).setAutoCancel(true).setGroupSummary(true);
		...
		if (unreadChatsCursor.getCount() > 1) {
			...
			while (unreadChatsCursor.moveToNext()) {
				...
				style.addLine(Html.fromHtml(
						"<b>" + unreadChatsCursor.getString(unreadChatsCursor.getColumnIndex(ChatProvider.FIELD_NAME)) +
								"</b>&nbsp;&nbsp;" + unreadChatsCursor.getString(
								unreadChatsCursor.getColumnIndex(ChatProvider.FIELD_LAST_MESSAGE))));
			}
			style.setBigContentTitle(context.getString(R.string.notification_new_messages, count));
			style.setSummaryText(
					context.getString(R.string.notification_unread_conversations, unreadChatsCursor.getCount()));
			...
			summaryResultIntent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TASK);
			summaryResultIntent.putExtra("open", "chats");
			summaryNotificationBuilder.setContentIntent(
					PendingIntent.getActivity(context, 0, summaryResultIntent, PendingIntent.FLAG_UPDATE_CURRENT));
		} else {
